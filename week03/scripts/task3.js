...
function subtract( minuend,  subrahend){
...
document.querySelector('#sumOfArray').textContent = numbers.reduce(number = (total, number )=> total + number, 0);
...
document.querySelector('#sumOfMultiplied').textContent = numbers.map(number => number *2).reduce(number = (sum, number) => sum + number, 0);
