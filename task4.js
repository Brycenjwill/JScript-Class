...
info.hobbies.forEach((item) => {
    let favHobby = document.createElement('li');
    favHobby.textContent = item;
    document.querySelector('#hobbies').appendChild(favHobby);
});
// Step 8: For each object in the <em>placesLived</em> property:
// - Create an HTML <dt> element and put its place property in the <dt> element
// - Create an HTML <dd> element and put its length property in the <dd> element
info.placesLived.forEach((location) => {
    let placeLived = document.createElement('dt')
    placeLived.textContent = location.place;
    let placeLivedLength = document.createElement('dd');
    placeLivedLength.textCon = location.length;
